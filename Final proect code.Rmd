...
barplot(a, main=paste("Study", all.t[1], "mclust classifications"))
}
...
data.norm = lapply(1:length(dat), function(i) {datn1 <-as.data.frame(data.uncat[[i]])
                                              ctsn=log2(datn1+1)
                                                  return(ctsn)})

#=data.norm <- lapply(data.uncat, ****) # QUESTION: what was the norm transform function --normTransform(object, f = log2, pc = 1)
...
>>>>>>> c1438f2c38d7667fd23a448dc4b99c8af2703df4
...
# length(batch.id.unlist)
# ncol(data.ncat)
filtered.dat.list <- lapply(1:length(dat), function(i) data.ready(dat[[i]]))
data.bmc <- ComBat(data.ncat, batch.id.unlist, par.prior = TRUE, prior.plots = FALSE,
