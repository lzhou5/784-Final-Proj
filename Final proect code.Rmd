...
data.ncat <- Reduce(function(x,y) merge(x,y,all=T, by=0), data.uncat)
```
...
#replace 11 by length(dat) once dataset 12 and 13 get fixed
data.norm = lapply(1:11, function(i) {datn1 <-as.data.frame(data.uncat[[i]])
                                              ctsn=log2(datn1+1)
                                                  return(ctsn)})
