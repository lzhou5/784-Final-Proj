...
<!-- In this task we are interested in transform the data by the method called `Batch Mean Centering`. Batch mean centering transforms the observation $x_{ij}^k$ from gene $i$, sample $j$ and study $k$ by the formula: -->
<!-- \[\hat{x}_{ij}^k = x_{ij}^k - \bar{x}_i^k\] -->
<!-- Each study is first transformed by doing a $\log2$ normalization and then we take the mean $\bar{x}_i^k$ within each gene and each study. After that we use the original observation to subtract the mean to finish the transformation. -->
