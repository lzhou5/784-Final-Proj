...
From the Figure above, we can see that the heuristic clustering and the mclustering give similar results, although there are some discrepancies on the blue and green clusters. Compared to the lower left graph in the last figure, we can see that the Combat method adjusts for the batch effects pretty well since there are no significant outlier clusters in the lower left graph in the above figure. The optimal cluster number given by BIC based on this transformation method is 6, which is closer to the true subtype number 5, while the paper's algorithm claims to detect 3 clusters. 
# Summary of Results 
